...
Ensure you have the following files in a single folder (e.g., named VIT_Health_Project):
...
Documentation Files: Problem Statement.pdf, Project Screenshots.pdf, Project report VIT_Health_app.pdf (For reference)
...
Run the following command to install the necessary packages using the provided text file:

pip install -r requirements.txt


(Alternatively: pip install streamlit fpdf)
...
Network URL: To view on a mobile device connected to the same Wi-Fi, use the Network URL displayed in the terminal.
...
Ensure Python is added to your system PATH. Try running: python -m streamlit run health_app.py

"FileNotFoundError (vit_logo.jpg / vit_campus.png)":
Ensure the image files are named exactly vit_logo.jpg and vit_campus.png and are in the same directory as health_app.py.

Database Reset:
If you wish to clear all data, delete the vit_health_v6.db file that appears after running the app. A fresh one will be created upon restart.
